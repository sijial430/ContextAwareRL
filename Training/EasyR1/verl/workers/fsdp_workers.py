...
class FSDPWorker(Worker):
    ...
    def _process_multi_modal_inputs(self, data: DataProto):
        ...
        if cached_uid is not None:
            ...
            if len(cached_uid) != len(new_uid) or not np.all(new_uid == cached_uid):
                self._cache.clear()
    ...
    def update_actor(self, data: DataProto):
        ...
        # extract choose-image batch from meta_info (not split by DP_COMPUTE_PROTO dispatch)
        choose_image_raw = data.meta_info.pop("choose_image_batch", None)
        choose_image_data = None
        if choose_image_raw is not None:
            ci_dict = {}
            for k, v in choose_image_raw["tensors"].items():
                ci_dict[k] = torch.tensor(v)
            for k, v in choose_image_raw["non_tensors"].items():
                ci_dict[k] = v
            choose_image_data = DataProto.from_single_dict(ci_dict)
            choose_image_data.meta_info["min_pixels"] = data.meta_info.get("min_pixels")
            choose_image_data.meta_info["max_pixels"] = data.meta_info.get("max_pixels")
            choose_image_data.meta_info["video_fps"] = data.meta_info.get("video_fps")
            choose_image_data.non_tensor_batch["uid"] = np.array(
                [f"ci_{i}" for i in range(len(choose_image_data))], dtype=object
            )
            self._process_multi_modal_inputs(choose_image_data)
            choose_image_data = choose_image_data.to(torch.cuda.current_device())
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy", logger=None) as timer:
                metrics = self.actor.update_policy(data=data, choose_image_data=choose_image_data)
