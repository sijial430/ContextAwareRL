...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        choose_image_dataloader: Optional[StatefulDataLoader] = None,
    ):
        self.tokenizer = tokenizer
        self.processor = processor
        self.train_dataloader = train_dataloader
        self.val_dataloader = val_dataloader
        self.choose_image_dataloader = choose_image_dataloader
        self.config = config
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            # DEBUG: print a few rollout outputs to check format
            if "responses" in gen_batch_output.batch:
                resp_ids = gen_batch_output.batch["responses"]
                n_print = min(4, resp_ids.size(0))
                for i in range(n_print):
                    text = self.tokenizer.decode(resp_ids[i], skip_special_tokens=False)
                    # truncate long outputs for readability
                    if len(text) > 500:
                        text = text[:500] + "...(truncated)"
                    print(f"[ROLLOUT DEBUG] sample {i}: {text}")
    ...
    def fit(self):
        ...
        self.data_iterator = iter(self.train_dataloader)
        if self.choose_image_dataloader is not None:
            self.choose_image_iterator = iter(self.choose_image_dataloader)
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                # pack choose-image batch into meta_info for joint update_actor
                if self.choose_image_dataloader is not None:
                    try:
                        ci_batch_dict = next(self.choose_image_iterator)
                    except StopIteration:
                        self.choose_image_iterator = iter(self.choose_image_dataloader)
                        try:
                            ci_batch_dict = next(self.choose_image_iterator)
                        except StopIteration:
                            print("[WARN] choose_image_dataloader is empty, skipping CI this step.")
                            ci_batch_dict = None

                    if ci_batch_dict is not None:
                        ci_batch = DataProto.from_single_dict(ci_batch_dict)
                        batch.meta_info["choose_image_batch"] = {
                            "tensors": {
                                "input_ids": ci_batch.batch["input_ids"].numpy(),
                                "attention_mask": ci_batch.batch["attention_mask"].numpy(),
                                "position_ids": ci_batch.batch["position_ids"].numpy(),
                            },
                            "non_tensors": {
                                "ground_truth": ci_batch.non_tensor_batch["ground_truth"],
                                "multi_modal_data": ci_batch.non_tensor_batch.get(
                                    "multi_modal_data",
                                    np.array([None] * len(ci_batch), dtype=object),
                                ),
                            },
                        }
