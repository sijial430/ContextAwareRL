...
def _load_chat_template(path: str) -> str:
    """Load a chat template from a file. Supports both plain text and JSON files.

    For JSON files (ending with .json), extracts the 'chat_template' key.
    For other files (e.g. .jinja, .txt), reads the raw content.
    """
    with open(path, encoding="utf-8") as f:
        if path.endswith(".json"):
            data = json.load(f)
            if "chat_template" not in data:
                raise KeyError(f"JSON file {path} does not contain a 'chat_template' key.")
...
def get_tokenizer(model_path: str, override_chat_template: Optional[str] = None, **kwargs) -> PreTrainedTokenizer:
    ...
    if override_chat_template is not None:
        tokenizer.chat_template = _load_chat_template(override_chat_template)
...
def get_processor(model_path: str, override_chat_template: Optional[str] = None, **kwargs) -> Optional[ProcessorMixin]:
    ...
    if override_chat_template is not None:
        processor.chat_template = _load_chat_template(override_chat_template)
