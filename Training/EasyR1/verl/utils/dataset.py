...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        # filter by data_type before overlong filtering
        if data_type == "regular":
            self.dataset = self.dataset.filter(
                lambda ex: len(ex.get(image_key, [])) <= 1,
                desc="Filtering regular (single-image) samples",
            )
        elif data_type == "choose_image":
            self.dataset = self.dataset.filter(
                lambda ex: len(ex.get(image_key, [])) > 1 and ex.get(answer_key, "") in ("A", "B"),
                desc="Filtering choose-image samples",
            )
    ...
    def _build_messages(self, example: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if self.data_type == "choose_image":
            prompt_str = prompt_str + "\nOutput A or B directly."
    ...
    def __getitem__(self, index):
        ...
        example["question_type"] = self.data_type or "regular"
        return example
