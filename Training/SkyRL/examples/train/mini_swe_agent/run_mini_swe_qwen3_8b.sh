#!/bin/bash
#SBATCH --job-name=mini_swe_qwen3_8b_ct_v2
#SBATCH --output=logs/mini_swe_qwen3_8b_ct_v2.out
#SBATCH --error=logs/mini_swe_qwen3_8b_ct_v2.err
#SBATCH --nodes=1
...
# Colocated GRPO + choose-trajectory DPO aux loss for Qwen3-8B-SFT
...
#
# sbatch examples/train/mini_swe_agent/run_mini_swe_qwen3_8b_ct_v2.sh
...
CKPT_PATH="ckpts/llm_mini_swe_qwen3_8b_ct_v2"
EXPORT_PATH="exports/mini_swe_qwen3_8b_ct_v2"
MINISWE_TRAJ_DIR="traj/mini_swe_qwen3_8b_ct_v2"
...
.venv/bin/python -m examples.train.mini_swe_agent.main_mini_swe_ct \
  data.train_data="['$DATA_DIR/train_valid_best.parquet']" \
  data.val_data="['$DATA_DIR/validation_valid.parquet']" \
  ...
  trainer.policy.model.path="/path/to/Qwen3-8B" \
  ...
  trainer.run_name="mini_swe_qwen8B_ct_v2" \
