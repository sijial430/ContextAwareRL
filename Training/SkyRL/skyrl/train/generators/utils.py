...
# Cache for the probe below: keyed by (id(tokenizer), chat_template). A tokenizer
# instance's chat template is stable at training time, so we only need to probe
# once. This matters because `encode_messages_subset` is called per-message
# inside `get_response_ids_and_loss_mask_from_messages`, which runs per
# trajectory — without caching, each trajectory would do 2 * N extra
# apply_chat_template calls just to decide whether to take the sentinel path.
_GEN_PROMPT_THINK_CACHE: Dict[Tuple[int, Optional[str]], bool] = {}


def _tokenizer_gen_prompt_has_think(tokenizer, chat_template: Optional[str]) -> bool:
    """Return True iff the tokenizer's generation prompt embeds a `<think>` tag.

    Qwen3-8B-style templates: gen prompt is `<|im_start|>assistant\\n` (no <think>).
    Qwen3.5-A3B-style templates: gen prompt is `<|im_start|>assistant\\n<think>\\n`.
    """
    key = (id(tokenizer), chat_template)
    if key in _GEN_PROMPT_THINK_CACHE:
        return _GEN_PROMPT_THINK_CACHE[key]
    try:
        _probe_base = tokenizer.apply_chat_template(
            [{"role": "user", "content": ""}],
        ...
        _probe_with_gen = tokenizer.apply_chat_template(
            [{"role": "user", "content": ""}],
            add_generation_prompt=True,
            tokenize=True,
            chat_template=chat_template,
        ...
        _gen_prompt_text = tokenizer.decode(
            _probe_with_gen[len(_probe_base):], skip_special_tokens=False
        )
        result = "<think>" in _gen_prompt_text
    except Exception:
        result = False
    _GEN_PROMPT_THINK_CACHE[key] = result
    return result
...
def encode_messages_subset(messages: ConversationType, tokenizer, chat_template: Optional[str] = None):
    ...
    # Qwen3-style chat templates auto-inject an empty `<think>\n\n</think>\n\n`
    # block into the *last* assistant message when that message has no explicit
    # `</think>` content. Because we always tokenize `base + messages` with the
    # target assistant at `loop.last`, those phantom think tokens would be fed
    # into the loss mask as if the model had generated them. To suppress this
    # auto-injection, append a sentinel user message so the target assistant is
    # no longer the last turn, then strip the sentinel's trailing tokens.
    #
    # We only take this path when it matters:
    #   (a) the last message is an assistant message without an explicit
    #       `</think>` block (so the template *would* auto-inject), AND
    #   (b) the tokenizer's generation prompt does not already embed `<think>`.
    #
    # Condition (b) distinguishes Qwen3-8B-style templates (gen prompt =
    # `<|im_start|>assistant\n`, think block injected only at loop.last) from
    # Qwen3.5-A3B-style templates (gen prompt = `<|im_start|>assistant\n<think>\n`,
    # think block is always part of the assistant turn structure). For the
    # latter, the sentinel would strip the legitimate `<think>` prefix and
    # break downstream generation-prompt matching.
    gen_prompt_has_think = _tokenizer_gen_prompt_has_think(tokenizer, chat_template)

    needs_sentinel = (
        messages[-1].get("role") == "assistant"
        and "</think>" not in (messages[-1].get("content") or "")
        and not gen_prompt_has_think
    )

    if needs_sentinel:
        sentinel_user = {"role": "user", "content": "x"}
        full_with_sentinel_ids = tokenizer.apply_chat_template(
            base_conversation + list(messages) + [sentinel_user],
            add_generation_prompt=False,
            tokenize=True,
            chat_template=chat_template,
            return_dict=False,
        )
        base_plus_sentinel_ids = tokenizer.apply_chat_template(
            base_conversation + [sentinel_user],
            add_generation_prompt=False,
            tokenize=True,
            chat_template=chat_template,
            return_dict=False,
        )
        sentinel_tail_len = len(base_plus_sentinel_ids) - len(base_conversation_token_ids)
        conversation_token_ids = full_with_sentinel_ids[
            len(base_conversation_token_ids) : len(full_with_sentinel_ids) - sentinel_tail_len
        ]
...
def get_response_ids_and_loss_mask_from_messages(
    ...
    for i in range(len(messages)):
        ...
        if cur_message["role"] == "user":
            ...
            #
            # Note: Due to tokenizer merging (e.g., `\n` from generation prompt merging with
            # leading `\n` in content), exact token ID matching may fail. Fall back to
            # text-based matching to find the generation prompt boundary.
            if cur_token_ids[: len(generation_prompt_ids)] == generation_prompt_ids:
                gen_prompt_len = len(generation_prompt_ids)
            else:
                # Fallback: find the generation prompt boundary by decoding and matching text
                gen_prompt_text = tokenizer.decode(generation_prompt_ids, skip_special_tokens=False)
                cur_text = tokenizer.decode(cur_token_ids, skip_special_tokens=False)
                if cur_text.startswith(gen_prompt_text):
                    # Re-tokenize just the generation prompt portion to find the correct length
                    gen_prompt_len = len(tokenizer.encode(gen_prompt_text, add_special_tokens=False))
                    # Adjust: the merged tokens may have different length, find exact boundary
                    # by scanning for where the content starts
                    content_text = cur_text[len(gen_prompt_text):]
                    prefix_ids = tokenizer.encode(gen_prompt_text + content_text[:1], add_special_tokens=False)
                    # Find the split point where generation prompt ends
                    for k in range(1, len(cur_token_ids)):
                        decoded_prefix = tokenizer.decode(cur_token_ids[:k], skip_special_tokens=False)
                        if len(decoded_prefix) >= len(gen_prompt_text):
                            gen_prompt_len = k
                            break
                else:
            ...
            if tokenizer.eos_token_id in cur_token_ids:
                ...
                generated_token_ids = cur_token_ids[gen_prompt_len : last_eos_token_index + 1]
                tokens_after_eos = cur_token_ids[last_eos_token_index + 1 :]
            else:
                generated_token_ids = cur_token_ids[gen_prompt_len :]
                tokens_after_eos = []
            assert gen_prompt_len + len(generated_token_ids) + len(tokens_after_eos) == len(
                cur_token_ids
            ), "The sum of the lengths of the generation prompt, the generated tokens, and the tokens after the EOS token should equal the length of the current token IDs"

            # 3.2.1. Add the generation prompt IDs.
            loss_mask.extend([0] * gen_prompt_len)
            if assistant_logprobs:
                rollout_logprobs.extend([0.0] * gen_prompt_len)
