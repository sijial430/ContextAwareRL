...
class RayPPOTrainer:
        ...
        for epoch in range(start_epoch, self.cfg.trainer.epochs):
            ...
            for iter, rand_prompts in enumerate(self.train_dataloader):
                with Timer("step", self.all_timings):
                    ...
                    _pre_generate_ts = time.time()
                    with Timer("generate", self.all_timings):
                        generator_output: GeneratorOutput = await self.generate(generator_input)
                    # Clean up stale uv build cache dirs created by Ray workers during generation.
                    # uv run --isolated creates .tmp* dirs in builds-v0/ but never removes them,
                    # which can exhaust file-count quotas within a single training run.
                    self._cleanup_uv_build_cache(_pre_generate_ts)
                    ...
                    # accumulate epoch-level reward
                    if "reward/avg_raw_reward" in self.all_metrics:
                        epoch_reward_sum += self.all_metrics["reward/avg_raw_reward"]
            ...
            # log epoch-level average reward
            if epoch_reward_count > 0:
                epoch_avg_reward = epoch_reward_sum / epoch_reward_count
                logger.info(f"Epoch {epoch} average reward: {epoch_avg_reward:.4f}")
                self.tracker.log(
                    {"reward/epoch_avg_raw_reward": epoch_avg_reward, "trainer/epoch": epoch},
    ...
    @torch.no_grad()
    def _cleanup_uv_build_cache(self, created_after: float) -> None:
        """Remove stale uv isolated-run temp dirs created by Ray workers.

        Args:
            created_after: only remove dirs whose mtime is after this UNIX timestamp
                (i.e. dirs created during the most recent generate phase).
        """
        builds_dir = os.path.join(os.path.expanduser("~"), ".cache", "uv", "builds-v0")
        if not os.path.isdir(builds_dir):
            return
        removed = 0
        for entry in glob.glob(os.path.join(builds_dir, ".tmp*")):
            try:
                if os.path.isdir(entry) and os.path.getmtime(entry) >= created_after:
                    shutil.rmtree(entry, ignore_errors=True)
                    removed += 1
            except OSError:
                pass
        if removed:
            logger.info(f"Cleaned up {removed} stale uv build cache dirs")
