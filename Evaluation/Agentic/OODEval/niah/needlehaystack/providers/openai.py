...
class OpenAI(ModelProvider):
    ...
    def __init__(self,
        ...
        # NIAH_MODEL_BASE_URL points the model-under-test at a local vLLM server.
        # Kept separate from OPENAI_BASE_URL so the judge can still hit real OpenAI.
        self.model = AsyncOpenAI(api_key=self.api_key, base_url=os.getenv("NIAH_MODEL_BASE_URL"))
        # If NIAH_MODEL_TOKENIZER_PATH points at a local HF model, use its tokenizer
        # (avoids tiktoken needing network access to fetch encodings on a sandboxed cluster).
        hf_path = os.getenv("NIAH_MODEL_TOKENIZER_PATH")
        if hf_path:
            from transformers import AutoTokenizer
            hf_tok = AutoTokenizer.from_pretrained(hf_path)
            class _HFShim:
                def __init__(self, t): self._t = t
                def encode(self, text): return self._t.encode(text, add_special_tokens=False)
                def decode(self, ids):  return self._t.decode(ids, skip_special_tokens=True)
            self.tokenizer = _HFShim(hf_tok)
        else:
            try:
